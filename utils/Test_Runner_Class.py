...
class Test_Runner_Class:
    ...
    def __init__(self, base_url='http://qxf2.com', testrail_flag='N', browserstack_flag='N',
        ...
        self.test_directory = os.path.abspath(
            os.path.join(os.path.dirname(__file__), '..', 'tests'))
        self.setup_testrail_script = os.path.join(
            self.util_directory, "setup_testrail.py")
    ...
    def run_setup_testrail(self, test_name=None, test_run_name='',
                           case_ids_list=None, name_override_flag=True):
        "Run the setup_testrail with given command line options"
        if self.testrail_flag.lower() == 'y':
            testrail_args_list = self.setup_testrail_args_list(
                test_name, test_run_name, case_ids_list, name_override_flag)
