            ...
            if self.connect():
                for command in commands:
                    ...
                    if self.ssh_error:
                        ...
                        print(
                            "Command execution completed successfully", command)
                    self.client.close()
            else:
...
if __name__ == '__main__':
    ...
    if ssh_obj.upload_file(ssh_obj.uploadlocalfilepath,
                           ssh_obj.uploadremotefilepath) is True:
        print("File uploaded successfully", ssh_obj.uploadremotefilepath)
    else:
        print("Failed to upload the file")

    # Sample code to download a file from the server
    if ssh_obj.download_file(ssh_obj.downloadremotefilepath,
                             ssh_obj.downloadlocalfilepath) is True:
