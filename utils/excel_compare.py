            ...
            if (len(actual_file) != len(exp_file)):
                ...
                for actual_row, actual_col in zip(actual_file, exp_file):
                    if actual_row == actual_col:
                        ...
                        print(
                            "Mismatch between actual and expected file at position(each row consists of 23 coordinates):",
                            actual_file.index(actual_row))
                        print(
                            "Data present only in Actual file: %s" %
                            actual_row)
                        print(
                            "Data present only in Expected file: %s" %
                            actual_col)
                        result_flag = False
