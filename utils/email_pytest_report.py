...
class Email_Pytest_Report:
        ...
        if html_body_flag == True and report_file_path == 'default':
            ...
            test_report_file = os.path.abspath(
                os.path.join(
                    ...
                    '..',
                    'log',
                    'pytest_report.html'))  # Change report file name & address here
        elif html_body_flag == False and report_file_path == 'default':
            # To generate pytest_report.log file add ">pytest_report.log" at
            # end of py.test command e.g. py.test -k example_form -r F -v >
            ...
            test_report_file = os.path.abspath(
                os.path.join(
                    ...
                    '..',
                    'log',
                    'pytest_report.log'))  # Change report file name & address here
        else:
            test_report_file = report_file_path
        # check file exist or not
        if not os.path.exists(test_report_file):
            raise Exception(
                "File '%s' does not exist. Please provide valid file" %
                test_report_file)
    ...
    def get_attachment(self, attachment_file_path='default'):
        ...
        if attachment_file_path == 'default':
            # To generate pytest_report.html file use following command e.g.
            # py.test --html = log/pytest_report.html
            attachment_report_file = os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__),
                    '..',
                    'log',
    ...
    def send_test_report_email(
        ...
        if html_body_flag == True and attachment_flag == False:
            ...
            # Add html formatted test data to email
            message = MIMEText(testdata, "html")

        # 2. Get text formatted email body data from report_file_path file
        # (log/pytest_report.log) and do not add it as an attachment
        elif html_body_flag == False and attachment_flag == False:
            # get html test report data from log/pytest_report.log
            testdata = self.get_test_report_data(
                html_body_flag, report_file_path)
            # Add text formatted test data to email
        ...
        # 4. Add text formatted email body message along with an attachment
        # file
        else:
            message = MIMEMultipart()
...
if __name__ == '__main__':
    ...
    # Here log/pytest_report.html is a default file. To generate
    # pytest_report.html file use following command to the test e.g. py.test
    # --html = log/pytest_report.html
    email_obj.send_test_report_email(
        html_body_flag=True,
        attachment_flag=True,
        report_file_path='default')

    # Note: We commented below code to avoid sending multiple emails, you can
    # try the other cases one by one to know more about email_pytest_report
    # util.
