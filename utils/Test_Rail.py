...
class Test_Rail:
    ...
    def create_new_project(self, new_project_name,
                           project_description, show_announcement, suite_mode):
    ...
    def create_test_run(
        ...
        if project_id is not None and test_run_id is None:
            ...
            if milestone_name is not None:
                milestone_id = self.get_milestone_id(
            ...
            if project_id is None:
                print(
                    "Cannot add test run %s because Project %s was not found" %
                    (test_run_name, project_name))
    ...
    def delete_project(self, new_project_name, project_description):
        ...
        if project_id is not None:
            ...
            print(
                'Cant delete the project given project name: %s' %
                (new_project_name))
    ...
    def update_testrail(self, case_id, run_id, result_flag, msg=""):
        ...
        # status_id is 1 for Passed, 2 For Blocked, 4 for Retest and 5 for
        # Failed
