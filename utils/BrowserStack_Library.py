...
class BrowserStack_Library():
    ...
    def get_session_url(self):
        ...
        session_url = self.browserstack_url + \
            'builds/%s/sessions/%s' % (build_id, session_id)
    ...
    def get_session_logs(self):
        ...
        build_id = self.get_build_id()
        session_id = self.get_active_session_id()
        session_log = requests.get(
            self.browserstack_url + 'builds/%s/sessions/%s/logs' %
            (build_id, session_id), auth=self.auth).text
    ...
    def get_latest_screenshot_url(self):
        ...
        # 2016-2-9 4:42:45:957 RESPONSE
        # {"state":"success","sessionId":"f77e1de6e4f42a72e6a6ecfd80ed07b95036ca35","hCode":19687124,"value":"New
        # Member Registration & Signup -
        # Chess.com","class":"org.openqa.selenium.remote.Response","status":0}
