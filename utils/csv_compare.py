    ...
    def is_equal(self, csv_actual, csv_expected):
        ...
        if os.path.exists(csv_actual) and os.path.exists(csv_expected):
            ...
            if (len(actual_file) != len(exp_file)):
                result_flag = False
                print(
                    "Mismatch in number of rows. The actual row count didn't match with expected row count")
            else:
                for actual_row, actual_col in zip(actual_file, exp_file):
                    if actual_row == actual_col:
                        ...
                        print(
                            "Mismatch between actual and expected file at Row: ",
                            actual_file.index(actual_row))
                        print(
                            "Row present only in Actual file: %s" %
                            actual_row)
                        print(
                            "Row present only in Expected file: %s" %
                            actual_col)
                        result_flag = False
