...
class Email_Util:
    ...
    def get_latest_email_uid(
            self, subject=None, sender=None, time_delta=10, wait_time=300):
        ...
        if subject is not None and sender is not None:
            search_string = '(FROM "{sender}" SUBJECT "{subject}")'.format(
                sender=sender, subject=subject)

        print(
            "  - Automation will be in search/wait mode for max %s seconds" %
            wait_time)
...
if __name__ == '__main__':
    ...
    # A. Look for an Email from provided sender, print uid and check it's
    # contents
    uid = email_obj.get_latest_email_uid(
        sender="Andy from Google", wait_time=300)
    if uid is not None:
    ...
    # B. Look for an Email with provided subject, print uid, find Qxf2 POM
    # address and compare with expected address
    uid = email_obj.get_latest_email_uid(
        subject="Qxf2 Services: Public POM Link", wait_time=300)
    if uid is not None:
        ...
        if data_flag == True and expected_pom_url == pom_url:
            ...
            print(
                "FAIL: Actual POM url not matched with expected pom url. Actual URL got from email: %s" %
                pom_url)
    ...
    uid = email_obj.get_latest_email_uid(
        subject="get more out of your new Google Account",
        sender="andy-noreply@google.com",
        wait_time=300)
    if uid is not None:
        print(
            "PASS: Unique id of the latest email with given subject and sender is: ",
            uid)
    else:
        print("FAIL: After wait of 5 mins, looks like there is no email present with given subject and sender")

    # D. Look for an Email with non-existant sender and non-existant subject
    # details
    uid = email_obj.get_latest_email_uid(
        subject="Activate your account",
        sender="support@qxf2.com",
        wait_time=120)  # you can change wait time by setting wait_time variable
    if uid is not None:
        print(
            "FAIL: Unique id of the latest email with non-existant subject and non-existant sender is: ",
            uid)
    else:
