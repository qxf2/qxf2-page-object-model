...
* To run the script in Gitbash, use the command 'python -u utils/xpath_util.py'
"""
...
class Xpath_Util:
                        ...
                        if element.has_attr(attr):
                            ...
                            if len(driver.find_elements_by_xpath(locator)) == 1:
                                ...
                                if variable_name != '' and variable_name not in self.variable_names:
                                    ...
                                    print(locator.encode('utf-8').decode('latin-1') + "----> Couldn't generate an appropriate variable name for this XPath")
                            ...
                            if element.getText() == button_text.strip():
                                ...
                                locator = self.guess_xpath_using_contains(guessable_element, "text()", button_text.strip())
                            if len(driver.find_elements_by_xpath(locator)) == 1:
                                result_flag = True
                                ...
                                if button_text.lower() not in self.button_text_lists:
                                    ...
                                    print(locator.encode('utf-8').decode('latin-1') + "----> Couldn't generate an appropriate variable name for this XPath")
                                break
                        elif guessable_element not in self.guessable_elements:
    ...
    def get_variable_names(self, element):
        ...
        if element.has_attr('id') and len(element['id']) > 2 and not re.search(r'\d', element['id']):
        ...
        # condition to check if the 'value' attribute exists and not having date and time values in it.
        elif element.has_attr('value') and element['value'] != '' and not re.search(r'([\d]+([/-]|\s|[.])?)+(\D+)?([/-]|\s|[.])?[[\d]+', element['value']) and not re.search(r'\d{1,2}[:]\d{1,2}\s+((am|AM|pm|PM)?)', element['value']):
            ...
            if element.has_attr('type') and element['type'] in ('radio', 'submit', 'checkbox', 'search'):
                if element.getText() != '':
                    self.variable_name = f"{element['type']}_{element.getText().strip('_.')}"
                else:
                    self.variable_name = f"{element['type']}_{element['value'].strip('_.')}"
            else:
        ...
        elif element.has_attr('placeholder') and not re.search(r'\d', element['placeholder']):
...
if __name__ == "__main__":
    ...
    if not xpath_obj.generate_xpath(soup, driver):
