...
class Xpath_Util:
    ...
    def generate_xpath(self, soup):
        ...
        try:
                        ...
                        for attr in self.known_attribute_list:
                            if element.has_attr(attr):
                                locator = self.guess_xpath(
                                    guessable_element, attr, element)
                                if len(driver.find_elements_by_xpath(
                                    ...
                                    if variable_name != '' and variable_name not in self.variable_names:
                                        ...
                                        print(
                                            locator.encode('utf-8') +
                                            "----> Couldn't generate appropriate variable name for this xpath")
                                        break
                                ...
                                if len(driver.find_elements_by_xpath(
                                        locator)) == 1:
                                    result_flag = True
                                    # Check for ascii characters in the
                                    # button_text
                                    matches = re.search(
                                        r"[^\x00-\x7F]", button_text)
                                    if button_text.lower() not in self.button_text_lists:
                                        self.button_text_lists.append(
                                            button_text.lower())
                                        if not matches:
                                            # Striping and replacing characters
                                            # before printing the variable name
                                            print(
                                                "%s_%s = %s" %
                                                (guessable_element,
                                                 button_text.strip().strip("!?.").encode('utf-8').lower().replace(
                                                     " + ",
                                                     "_").replace(
                                                     " & ",
                                                     "_").replace(
                                            ...
                                            # printing the variable name with
                                            # ascii characters along with
                                            # language counter
                                            print(
                                                "%s_%s_%s = %s" %
                                                (guessable_element,
            ...
            print(
                "Exception when trying to generate xpath for:%s" %
                guessable_element)
    ...
    def get_variable_names(self, element):
        ...
        # date and time values in it.
            ...
            # for the rest of the type attributes printing the 'type'+'value'
            # attribute values. Doing a check to see if 'value' and 'type'
            # attributes values are matching.
            if (element.has_attr('type')) and (element['type'] in (
                    'radio', 'submit', 'checkbox', 'search')):
                if element.getText() != '':
                    self.variable_name = element['type'] + \
                        "_" + element.getText().strip().strip("_.")
                else:
                    self.variable_name = element['type'] + \
                        "_" + element['value'].strip("_.")
            else:
                if element['type'].lower() == element['value'].lower():
                    ...
                    self.variable_name = element['type'] + \
                        "_" + element['value'].strip("_.")
        # condition to check if the "name" attribute exists and if the length
        # of "name" attribute is more than 2 printing variable name
        elif element.has_attr('name') and len(element['name']) > 2:
            self.variable_name = element['name'].strip("_")
        # condition to check if the "placeholder" attribute exists and is not
        # having any numerics in it.
        elif element.has_attr('placeholder') and bool(re.search(r'\d', element['placeholder'])) == False:
            self.variable_name = element['placeholder'].strip(
                "_?*.").encode('ascii', errors='ignore')
    ...
    def guess_xpath(self, tag, attr, element):
        ...
        # Class attribute returned as a unicodeded list, so removing 'u from
        # the list and joining back
    ...
    def guess_xpath_using_contains(self, tag, attr, element):
        ...
        self.button_contains_xpath = "//%s[contains(%s,'%s')]" % (
            tag, attr, element)
...
if __name__ == "__main__":
    ...
    page = driver.execute_script("return document.body.innerHTML").encode(
