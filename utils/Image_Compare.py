...
def rmsdiff(im1, im2):
    ...
    # calculate rms
    return math.sqrt(sum(h * (i**2) for i, h in enumerate(h)
    ...
    if os.path.exists(img_actual) and os.path.exists(img_expected):
        ...
        if (ImageChops.difference(actual, expected).getbbox() is None):
            ...
            if result_flag is False:
                ...
                # Now with threshhold!
                result_flag = True if rmsdiff(
                    actual, expected) < 958 else False
...
def analyze_difference_smartly(img):
    ...
    if not os.path.exists(img):
        print(
            'Could not locate the image to analyze the difference smartly: %s' %
            img)
    else:
        ...
        print(
            'Number of different pixels in the result image: %d' %
            num_different_pixels)
        # Rule 1: If the number of different pixels is <10, then pass the image
        # This is relatively safe since all changes to objects will be more
        # than 10 different pixels
