...
class Bitcoin_Main_Page(Mobile_Base_Page):
    ...
    def click_on_price_button(self):
        ...
        try:
            ...
            self.write(
                "Exception while clicking on the bitcoin real time price button.")
            self.write(str(e))
    ...
    def check_redirect(self, expected_bitcoin_price_page_heading):
        ...
        result_flag = False
        bitcoin_price_page_heading = self.get_text_by_locator(
            self.bitcoin_price_page_heading)
        if bitcoin_price_page_heading.decode(
                'utf-8') == expected_bitcoin_price_page_heading:
            result_flag = True
    ...
    @Wrapit._screenshot
    def click_on_real_time_price_button(
            self, expected_bitcoin_price_page_heading):
