...
class Mobile_Base_Page(Borg, unittest.TestCase):
    ...
    def click_element(self, locator, wait_time=3):
        ...
        if link is not None:
            try:
                ...
                self.write(
                    'Exception when clicking link with path: %s' %
                    locator)
                self.write(e)
            else:
                return True
    ...
    def set_text(self, locator, value, clear_flag=True):
        ...
        try:
            ...
            self.write(
                'ERROR: Could not clear the text field: %s' %
                locator, 'debug')
    ...
    def conditional_write(self, flag, positive, negative,
                          level='debug', pre_format="  - "):
