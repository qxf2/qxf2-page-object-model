...
class DriverFactory():
        ...
        if (remote_flag.lower() == 'y'):
            try:
                if remote_credentials.REMOTE_BROWSER_PLATFORM == 'BS':
                    web_driver = self.run_browserstack(
                        os_name,
                        os_version,
                        browser,
                        browser_version,
                    ...
                    web_driver = self.run_sauce_lab(
                        os_name, os_version, browser, browser_version)

            except Exception as e:
                print(
                    "\nException when trying to get remote webdriver:%s" %
    ...
    def run_browserstack(self, os_name, os_version, browser,
                         browser_version, remote_project_name, remote_build_name):
        ...
        if remote_build_name is not None:
            desired_capabilities['build'] = remote_build_name + \
                "_" + str(datetime.now().strftime("%c"))
    ...
    def run_mobile(self, mobile_os_name, mobile_os_version, device_name,
                   app_package, app_activity, remote_flag, device_flag, app_name, app_path):
        "Setup mobile device"
        ...
        if (remote_flag.lower() == 'y'):
            ...
            try:
                if remote_credentials.REMOTE_BROWSER_PLATFORM == 'SL':
                    # Saucelabs expects the app to be uploaded to Sauce storage
                    # everytime the test is run
                    self.sauce_upload(app_path, app_name)
                    # Checking if the app_name is having spaces and replacing
                    # it with blank
                    if ' ' in app_name:
                    ...
                    desired_capabilities['app'] = self.browser_stack_upload(
                        app_name, app_path)  # upload the application to the Browserstack Storage
            ...
            try:
                ...
                if device_flag.lower() == 'y':
                    driver = mobile_webdriver.Remote(
                        'http://localhost:4723/wd/hub', desired_capabilities)
                else:
                    desired_capabilities['app'] = os.path.join(
                        app_path, app_name)
                    driver = mobile_webdriver.Remote(
                        'http://localhost:4723/wd/hub', desired_capabilities)
            except Exception as e:
    ...
    def sauce_upload(self, app_path, app_name):
        ...
        try:
            ...
            data = fp.read()
            fp.close()
            # Checking if the app_name is having spaces and replacing it with
            # blank
            if ' ' in app_name:
                app_name = app_name.replace(' ', '')
                print(
                    "The app file name is having spaces, hence replaced the white spaces with blank in the file name:%s" %
                    app_name)
            response = requests.post(
                'https://saucelabs.com/rest/v1/storage/%s/%s?overwrite=true' %
                (USERNAME, app_name), headers=headers, data=data, auth=(
                    USERNAME, PASSWORD))
