...
class Bitcoin_Price_Page(Mobile_Base_Page):
    ...
    def get_bitcoin_real_time_price(self):
        ...
        # Final result flag is to return False if expected price and real time
        # price is different.
        try:
            # Get real time price of the bitcoin in usd.
            result_flag = None
            bitcoin_price_in_usd = self.get_text_by_locator(
                self.bitcoin_price_in_usd)
            ...
            self.write(
                "Exception while getting real time price of the bitcoin.")
            self.write(str(e))
