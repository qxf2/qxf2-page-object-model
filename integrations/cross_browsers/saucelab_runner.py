...
class SauceLabRunner(RemoteOptions):
        ...
        try:
            ...
            with open(apk_file_path, 'rb') as apk_file:
                ...
                if response.status_code == 201:
                    ...
                    raise Exception(f"Failed to upload APK file. Status code: {response.status_code}")
    ...
    def get_saucelab_mobile_driver(self, app_path, app_name, desired_capabilities):
        ...
        result_flag = self.sauce_upload(app_path, app_name) #Saucelabs expects the app to be uploaded to Sauce storage everytime the test is run

        if result_flag:
            desired_capabilities = self.saucelab_capabilities(desired_capabilities, app_name)
            mobile_driver = self.set_capabilities_options(desired_capabilities, url=self.saucelabs_url)
        else:
