...
def test_weather_main(test_obj):
    ...
    try:
        ...
        # #Update TestRail
        # case_id = testrail_file.test_weather_main
        # test_obj.report_to_testrail(case_id,test_obj.test_run_id,result_flag)
        # test_obj.add_tesults_case("Set email", "Sets the email in the form", "test_weather_main", result_flag, "Failed to set email: %s \nOn url: %s\n"%(email_address,test_obj.get_current_url()), [test_obj.log_obj.log_file_dir + os.sep + test_obj.log_obj.log_file_name])
        # #6. Set Email in form
        ...
        # test_obj.write('Script duration: %d seconds\n'%(int(time.time()-start_time)))
        # test_obj.add_tesults_case("Check Heading", "Checks the heading on the redirect page", "test_weather_main", result_flag,"Fail: Heading on the redirect page is incorrect!", [])
